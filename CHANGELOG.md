...
#### 💥 Breaking Changes
- **移除现货交易支持** - 项目现在专注于合约交易
- 删除 `main.py` (现货版)、`binance_client.py`、`copy_trade_engine.py`
- `main_futures.py` 重命名为 `main.py`
- `config.futures.example.yaml` 重命名为 `config.example.yaml`

#### 🔧 关键问题修复 (基于量化专家 Code Review)

##### P0 - 严重问题修复
1. **时间同步增强** ⭐
   - 失败后自动重试 3 次
   - 全部失败则抛出异常，避免签名错误
   - 修复可能导致所有请求失败的 -1021 错误

2. **余额检查并发安全** ⭐
   - 为每个 follower 添加独立的余额锁
   - 防止多个订单同时检查余额导致超额下单
   - 确保余额检查和下单的原子性

3. **MIN_NOTIONAL 检查完善** ⭐
   - MARKET 订单也检查 MIN_NOTIONAL
   - 使用 mark price 估算 MARKET 订单价值
   - 符合币安官方文档要求

##### P1 - 重要问题修复
4. **订单去重机制优化** ⭐
   - 从 `deque(maxlen=1000)` 改为时间戳字典
   - 自动清理 1 小时前的记录
   - 避免高频交易时的溢出问题

5. **初始化时设置杠杆** ⭐
   - 启动时预配置 `symbol_leverage` 中的所有交易对
   - 自动设置杠杆和保证金类型
   - 避免第一笔交易使用默认杠杆

6. **Position Side 处理改进** ⭐
   - One-way 模式强制使用 `BOTH`
   - Hedge 模式正确使用 `LONG`/`SHORT`
   - 符合币安持仓模式规范
