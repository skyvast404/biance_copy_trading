...
专为币安合约（Binance Futures）设计的高级跟单交易系统。
...
### 🚀 高级功能（已实现）
- ✅ **余额检查** - 下单前自动检查账户余额是否充足
- ✅ **MIN_NOTIONAL 验证** - 确保订单金额满足最小要求
- ✅ **精度处理** - 自动处理价格和数量精度（LOT_SIZE, PRICE_FILTER）
- ✅ **杠杆管理** - 自动设置和管理杠杆倍数
- ✅ **持仓模式** - 支持单向持仓和双向持仓（对冲模式）
- ✅ **保证金模式** - 支持全仓和逐仓模式
- ✅ **订单去重** - 防止 WebSocket 重复消息导致重复下单
- ✅ **部分成交** - 正确处理部分成交情况
...
```bash
python main_futures.py --config my_config.yaml
```
...
├── README_FUTURES.md                  # 合约版文档
└── src/
    ├── binance_futures_client.py      # 合约 API 客户端
    ├── futures_copy_trade_engine.py   # 合约跟单引擎
